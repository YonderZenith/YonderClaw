...
### Post-smoke fixes (2026-04-22, three push-blockers)
Second-agent fresh install (2026-04-22) surfaced three issues that had to land before v3.7.2 can publish. All three are root-caused and fixed.

- **Hyperswarm missing despite QIS tier-2 opt-in** (task #293). Valorie hit it 2026-04-20, second agent hit it 2026-04-22 — same pattern: `npm install` silently partial-failed (cache hiccup / timeout / resolver), the Listr task swallowed the error, and the agent booted without `node_modules/hyperswarm` even though `mergeDependencies` had correctly written the spec to `package.json`. Safety nets in `module-loader.ts::mergeDependencies` were correct on paper but fail open. Installer now (a) runs `npm install`, (b) enumerates every dep from the merged manifest, (c) walks `node_modules/<dep>/package.json` to verify each one landed, (d) retries just the missing deps by name-with-version, (e) if any are still missing, writes `data/.install-issues.json` (beacon the agent's reboot-prompt reads on wake) and throws from the Listr task so the operator sees the failure immediately instead of discovering it three launches later. Belt-and-suspenders for the silent-drop failure mode.
- **Agents didn't know how to customize their Tauri dashboard** (task #294). The `"dashboard"` section rendered into CLAUDE.md by `module-loader.ts::sectionRenderers` was stale v3.6.x copy: "npm run dashboard — regenerate dashboard.html, Dashboard opens in Edge app mode." Completely wrong for v3.7.2's Board → `data/dashboard-config.json` → Rust watcher → LayoutFrame flow. CT directive: "the ai needs to know the ins and out of customizing it." Rewrote the section to teach: file-is-source-of-truth editing, `scripts/dashboard-helper.cjs` subcommands (list/add/remove/move/theme), 9 panel types by name, dataKey binding into `data/dashboard-data.json`, per-claw theme invitation. Docs (`docs/dashboard-panels.md`) already shipped via `core-scaffold.ts:74-75` — the gap was CLAUDE.md not pointing at them.
- **QIS bucket creation wrongly required admin approval** (task #295). `relay-server.ts::POST /buckets/propose` queued any proposal in an unseeded domain for admin review, contradicting CT directive 2026-04-22: "all agents can make buckets as long as they checked same problem doesn't exist and follow the set protocols." Gate removed. New behavior: if path format is valid + PII passes + no exact collision + similarity < 80%, auto-approve. Close-match range (60–80%) still auto-approves but the response carries `closest_match` so the agent can consider using the existing bucket instead. `approved_domain_prefixes` retained for seed/telemetry, not gating. Matching copy updated in `module-loader.ts` swarm section + `installer/modules/swarm/QIS-RELAY-GETTING-STARTED.md`. **Global relay at 64.23.192.227:7891 requires redeploy** before the fix is live.
- **Post-audit cloud sync baked in** (Rule 5a, ratified by Brian 2026-04-22). The standalone `ContinuityCloudSync` cron dies silently on some fleet agents — today's audit caught 4 stale syncs 42-46h old and 3 with no current-identity backup. Chaining the sync off `PersistenceAudit` (higher-priority, self-logging) closes the cron-not-firing gap permanently: as long as the audit fires, the sync fires. Three scaffold changes land it for every new agent at install time — `installer/templates/cron-manager.ts.txt::createBatWrapper` conditionally appends the `continuity-cloud-sync.cjs` chain when `task.name === "PersistenceAudit"` (so `scripts/persistenceaudit.bat` runs the audit then immediately rclone-syncs Layer-2 ciphertext to the operator's R2/Proton/B2 remote); `installer/templates/persistence-audit.md.txt` adds Q10 (`CHANGES TO CLOUD — handled automatically by the post-audit hook`); `installer/core-scaffold.ts` Non-Negotiable Rule 5a tells agents the chain is contractual. New template `installer/templates/continuity-cloud-sync.cjs.txt` scaffolds to `scripts/continuity-cloud-sync.cjs`; it no-ops cleanly (exit 1, clear console error) until the operator does the one-time `winget install Rclone.Rclone` + `rclone config` + fills `data/continuity-cloud-config.json`. Idempotent — rclone transfers nothing when nothing changed, so the cost is ~20-30 s per hour even at steady state. Canonical spec: `Z:/shared/continuity-rollout-package/POST-AUDIT-CLOUD-SYNC-RULE.md`.
- **`qis.propose()` + auto-propose on 404** (tarball rev-4, sha d7f1fb98, 2026-04-22). Fresh-install agent (second-agent v3.7.2 smoke) opted into swarm (tier 2), tried to deposit, got back `404 Bucket does not exist. Propose it first via POST /buckets/propose` — then tried to propose and got `401 Auth required to propose buckets.` Root cause: `qis-client` exposed `qis.deposit()` but no `qis.propose()`, and the relay requires Ed25519-signed proposals — agent had no exposed code path to create its first bucket. Rev-2's admin-gate removal worked, but the auth requirement (which was always there, correctly) was the real wall. Fix in three files: (1) `qis-write-control.ts.txt::postJSON` now preserves `err.status` + `err.payload` on 4xx rejections so callers can branch on status; (2) new `proposeBucket()` function signs a canonical packet with `bucket=path`, synthetic `{signal:"neutral", confidence:0, insight:title}` outcome (matches exactly what `relay-server.ts::verifyEd25519Packet` reconstructs at lines 103-113) and posts to `/buckets/propose` with `path + bucket + domain + title + description + public_key + signature + ts`; (3) `deposit()` catches `err.status === 404` with message matching `/bucket does not exist/i`, calls `proposeBucket({path, title: last path segment, description: insight.slice(0, 240)})`, then retries deposit once. Idempotent — if another agent proposed the same bucket first, retry sees it and succeeds. (4) `qis-client.ts.txt` re-exports `qis.propose = proposeBucket`. (5) `core-scaffold.ts::buildQisClaudeMdSection` updated in both active + holder branches to teach "if bucket doesn't exist, deposit auto-proposes — or call qis.propose() explicitly." Second-agent re-smoke with rev-4 should complete first-deposit without intervention.
- **Installer opt-ins: QIS DHT holder + Hive auto-register** (tarball rev-3, sha f95d1468, 2026-04-22). Two gaps CT caught reviewing the v3.7.2 flow. (1) Swarm questionnaire offered tier-2 read+write but never exposed tier-3 holder mode, even though the code was fully wired (`dht-client.ts.txt::start(holder)` creates SQLite `dht-holder.db`, joins MEMBERSHIP_TOPIC as server, runs `holderSync()` every 30 min, answers peer `SYNC_REQUEST`). Added a conditional confirm question in `installer/questionnaire.ts` gated on `joinSwarm`: "Become a QIS holder node? …stores + serves protocol data (like a Bitcoin node but for structured knowledge). Uses ~50-500 MB disk, keeps a background DHT peer alive." Default `false` (don't surprise operators with background IO). When true, `core-scaffold.ts` computes `qisTier = 3`, `buildQisClaudeMdSection` emits a HOLDER-mode teach block (responsibilities: keep running, don't wipe data/, watch disk budget), and `qis-config.json` substitution writes `tier: 3` which `qis-client.ts.txt:107` reads to boot DHTClient with `holder: true`. (2) Hive auto-registration at `installer/index.ts:872` fired unconditionally on every install — some operators don't want their agent announced to `hive.yonderzenith.com`. Added an opt-in confirm (default `true` because it's the expected YonderClaw experience) and gated the existing emerge call behind `finalConfig.autoRegisterHive !== false` via Listr `skip:`. Both fields propagate through `questionnaire.answers → finalConfig.becomeQisHolder / autoRegisterHive`. No regressions — defaults match v3.7.2 rev-2 behavior (joinSwarm → tier 2, Hive registered) when operator accepts defaults.
